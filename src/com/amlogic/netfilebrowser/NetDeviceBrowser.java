...
;
	...
	private void openNetDevice(String pathname, String user, String password) {
    	...
    	ret = MntInfo.SmbUnMountWrapper("umount /mnt/NetShareDirs");
    	
    	if(ret == (int)0xabcd) {
    		showDialog(DIALOG_ERROR_MSG2, null);		    		
    	}
    	
    	ret = MntInfo.SmbMountWrapper(mtpath);
		...
		builder.setPositiveButton(R.string.confirm,
		    new DialogInterface.OnClickListener() {
			    public void onClick(DialogInterface dialog, int whichButton) {
					...
					if(MntInfo.SmbMountWrapper(mtpath) >= 0) {
						Intent intent = new Intent();
						intent.setClass(NetDeviceBrowser.this, NetFileBrowser.class);
						startActivity(intent);
						finish();
	...
	*/
	...
	private Handler searchHandler = new Handler() {
		public void handleMessage(Message msg) {
			switch(msg.what) {
				case 0:
					int tmpnum = MntInfo.SmbGetNumWrapper();
					if(tmpnum > 0) {
						searchCount = 0;
						stopSearchTime();
						stopSearchDialog();
